...
pAnalysis->Trig00[jk]=-1000000; pAnalysis->Trig01[jk]=-1000000; pAnalysis->Trig02[jk]=-1000000; pAnalysis->Trig03[jk]=-1000000;
  ...
  //   }
   ...
   unsigned int layid = 0;
    ...
    for(int ijk=0;ijk<7;ijk++){//loc_no loop 0:Top, 1:Left 2:Right 3:Back 4:Front 5: miniLeft 6:miniRight //4
      ...
      cout<<"locno: "<<ijk<<endl;
      if(ijk==0){ijmax=4;xhat=0;yhat=0;zhat=1;NoScintStrip=NoScntStrpTop;}//top
      else if(ijk==1 ){ijmax=3;xhat=-1;yhat=0;zhat=0;NoScintStrip=NoScntStrpSide; }//left
      else if(ijk==2){ijmax=3;xhat=1;yhat=0;zhat=0;NoScintStrip=NoScntStrpSide; }//right
      else if(ijk==3){ijmax=3;xhat=0;yhat=1;zhat=0;NoScintStrip=NoScntStrpSide;}//back
      else if(ijk==4){ijmax=3;xhat=0;yhat=-1;zhat=0;NoScintStrip=NoScntStrpSide;}//Front
      else if(ijk==5){ijmax=3;xhat=-1;yhat=0;zhat=0;NoScintStrip=NoScntStrpSideSmallay;}//miniLeft
      else if(ijk==6){ijmax=3;xhat=1;yhat=0;zhat=0;NoScintStrip=NoScntStrpSideSmallay;}//miniRight
	...
	// NoScintStrip =  (ijk==0) ? NoScntStrpTop:NoScntStrpSide;

        bool pl2 = LinePlaneInt (Line, Plane, G_Point);
        double layhalflength = 0.5*((2*NoScintStrip*partopscint[0])+(2*NoScintStrip*AirGapScintTop));//2289mm for top and 1041mm for sides
        double layhalfbreadth =partopscint[1];     //2300 for left right
	if(ijk==3){
	  layhalfbreadth = partopscint[1]+50;//4.7 m    

	}
		if(ijk==0){
	  layhalfbreadth =partopscint[1]-50;//4.5 m    

	}

	if(ijk==5 || ijk==6){
	  layhalfbreadth =   SidePlaneHalfLength+0.5*(2*ScntLayShifSide);
 ...
 layexp->SetId(layid);
	...
	layexp->Print();
		...
		cout<<"recohtvec: "<<tmphtvec<<endl;
        	double difx = (extposvec-tmphtvec).mag();//valgrind RSA
		...
		//	double difx = (extposvec-truehtvec).mag();
		  ...
		  cout<<"Reco-Extrap: "<<  tmphtvec.x()-G_Point[0]<<" "<<  tmphtvec.y()-G_Point[1]<<" "    <<tmphtvec.z()-G_Point[2]<<endl; //valgrind RSA
 ...
 cout<<"minimum closest dis from the 4 edges is: "<<minclosdist<<endl;
 	       layid = ijk+1;
  layid<<=2;
    layid+=ij;
 ...
 layexp->SetId(layid);
	...
	//	 }// for (unsigned iji=0; iji<CmvCluster_pointer->CmvCluster_list.size(); iji++) {
	      ...
	      cout<<"## "<<tmpcluster->GetPlane()-1<<" "<<tmpcluster->GetLayer()<<endl;
	       ...
	       cout<<"MinCloseDist:  "<<minclosdist<<" "<<hitpresent<<endl;
     if(hitpresent==true && abs(minclosdist)<300){                 
  ...
  CmvLayExtra_Manager *pCmvLayExtra = CmvLayExtra_Manager::APointer;
  
  if(pCmvLayExtra){
    
    if(pCmvLayExtra->CmvLayExtra_list.size()) {

    pAnalysis->pRootFile->cd();
      pAnalysis->cmv_nexphit = pCmvLayExtra->CmvLayExtra_list.size();
      cout<<"  pCmvLayExtra->CmvLayExtra_list.size():  "<<  pCmvLayExtra->CmvLayExtra_list.size()<<endl;
      cout <<"  pAnalysis->cmv_nexpthit "<<  pAnalysis->cmv_nexphit<<endl;

      for (unsigned int ijj=0; ijj<pCmvLayExtra->CmvLayExtra_list.size()/* && ij<pAnalysis->cmv_nclusthit*/; ijj++) {

	SaveCMVExtrapData(ijj,pCmvLayExtra->CmvLayExtra_list[ijj]); //valgrind RSA
  ...
  cout<<"CMVD_Extrapolation ended"<<endl;
}
